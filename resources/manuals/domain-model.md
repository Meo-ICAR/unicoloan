...
>
> **Autorizzazione a due livelli, entrambi devono passare**: le relation manager Filament basate sul trait `HasRelationPlanAccess` (Documenti, Siti Web, Sedi, ecc.) sono visibili solo se (1) `checkPiano($feature)` (piano attivo + `App\Enums\UserRole::features()`) **e** (2) `User::hasPermission($resource, $action)` sono entrambi `true`. Il secondo controllo è indipendente dal primo: richiede un `App\Models\Employee` collegato via `employees.user_id`, con `employee_roles` che abbia un `EmployeeType` con una riga corrispondente in `EmployeeTypePermission` — se l'utente non ha un Employee collegato, fallisce sempre, a prescindere dal ruolo. Fa eccezione il **superAdmin** (`User::role === 'admin'`): `hasPermission()` lo bypassa esplicitamente, perché per design non deve avere un profilo Employee. Se una scheda relation manager "non si vede" per un utente, controllare sempre entrambi i livelli, non solo la registrazione della classe in `getRelations()`.
>
> **🚧 TODO — Feature richiesta ma IN SOSPESO (bloccata: manca ancora la decisione su quali campi far inserire all'agente in fase di creazione pratica — non iniziare l'implementazione finché questo punto non è chiarito): accesso al pannello per gli agenti (Fornitore)**. Gli agenti devono poter accedere all'app (oggi solo `User`/dipendenti hanno un login) per: (1) inserire una nuova `Pratica` con i dati del cliente persona fisica e allegare i documenti di identità; (2) vedere lo stato delle proprie provvigioni (`ProvvigioniRule`/`Provvigione`) e delle pratiche ad esse collegate — solo le proprie, mai quelle di altri agenti. La pratica inserita da un agente passa poi a un `Employee` che la approva/rifiuta, inviando una email all'agente sull'avanzamento.
>
> **Verificato (settembre 2026): quando si seleziona/crea un `tipo_prodotto` su una Pratica, il sistema NON propone automaticamente le `ProvvigioniRule` applicabili — questa funzionalità non esiste da nessuna parte nel codice attuale.** La scheda "Provvigioni" su `PraticaResource` (`Praticas/RelationManagers/ProvvigioniRelationManager.php`) è di sola lettura e mostra `Provvigione` (compensi già calcolati/importati, tabella `provvigioni`), non `ProvvigioniRule` (le regole configurabili, tabella `provvigioni_rules`) — sono due modelli distinti, non collegati tra loro nel codice. Prima di costruire questa proposta automatica (sia lato admin sia per il futuro portale agenti), va decisa la logica di matching a cascata da applicare: `ProvvigioniRule` ha `tipoprodotto_id`/`tipoprodotto_sub_id`/`clienti_id`/`kind_id`/`fornitori_id`, tutti nullable — una regola più specifica (stessa combinazione ma con banca/agente valorizzati) dovrebbe prevalere su una più generica, ma questa priorità non è ancora implementata in nessuna query esistente.
>
> Decisioni di design già prese (in attesa di implementazione): login con `Fornitore.email` (non un'email raccolta ad hoc); **un solo account/login per Fornitore** (relazione 1:1, non 1:N — implica `users.fornitore_id` con vincolo di unicità, non solo nullable); un agente in blacklist verso una banca (`BlacklistClienteFornitore`/`BlacklistChecker`) va **bloccato in fase di creazione** della pratica per quella banca, non solo in approvazione; **nessun rate limiting** sull'upload documenti identità per ora (da rivalutare, essendo superficie pubblica). Architettura suggerita: nuovo Filament Panel dedicato con guard/autenticazione propria, scoping per-agente su ogni query (per stringa denormalizzata `denominazione_agente`, coerente con `Pratica::agente()`/`BlacklistChecker`, non una nuova FK), notifica email sul cambio di stato pratica tramite `EmailTemplate` esistente.
